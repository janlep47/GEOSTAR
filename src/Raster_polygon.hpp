...
*/
  ...
  private:
      ...
      template <typename T>
      void rasterToPolygonType();

      template <typename T>
      void OLDrasterToPolygonType();
      //Vector* rasterToPolygonType(Vector *vecNew);

      //template <typename T>
      //Raster* polygonToRasterType(Raster *rasNew);


  public:

      // FOR NOW, until get the GeoStar::Vector class ...
      void rasterToPolygon(const std::string &newVectorName);
      void rasterToPolygon();

      // FOR NOW, until get the GeoStar::Vector class ...
      void OLDrasterToPolygon(const std::string &newVectorName);
      void OLDrasterToPolygon();

      //Vector* rasterToPolygon(const std::string &newVectorName);
      //Vector* rasterToPolygon(Vector *vecNew);

      //Raster* polygonToRaster(const std::string &newRasterName);
      //Raster* polygonToRaster(Raster *rasNew);
